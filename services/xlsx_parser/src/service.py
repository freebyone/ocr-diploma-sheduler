...
def parse_code_from_filename(filename: str) -> str:
    ...
    raise ValueError(
        f"Не удалось извлечь code_file из имени файла: {filename}"
    )
...
def parse_reattest_sheet(filepath: str) -> List[Dict]:
    ...
    if not sheet_name:
        raise ValueError("Лист 'Переаттестация' не найден")
...
def save_excel_to_db(filepath: str, session: Session,
    ...
    for row_data in rows_data:
        ...
        control = ControlTable(
            ...
            format_retests_id=(
                format_retests.id if format_retests else None
            ),
...
def process_all_files_from_minio(session: Session):
    ...
    object_names = minio_client.list_xlsx_files()

    print(f"{'=' * 60}")
    print(f"Найдено xlsx-файлов в MinIO (бакет '{minio_client.bucket}'): "
          f"{len(object_names)}")
    print(f"{'=' * 60}\n")
...
def _print_summary(success: int, total: int,
                    errors: List[tuple]):
    print(f"\n{'=' * 60}")
...
def debug_sheet(filepath: str, sheet_name: str = 'Титул'):
    ...
    if not target:
        print(
            f"Лист '{sheet_name}' не найден. Доступные: {wb.sheetnames}"
        )
        return
