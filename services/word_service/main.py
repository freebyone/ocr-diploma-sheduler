...
def add_control_table(doc: Document, control_rows: List[ControlTable]):
    ...
    Создаёт таблицу:
    ┌──────────────────┬────────────────────┬─────────────────────────┬──────────────┬──────────────────┐
    │ Наименование     │ По учебному плану  │ Данные прилож. к диплому│ Переаттест.  │ Форма            │
    │ дисциплины       ├──────────┬─────────┼────────────┬────────────┤ (часов)      │ переаттестации    │
    │                  │ К-во час │ Форма   │ К-во час   │ Форма      │              │                  │
    ├──────────────────┼──────────┼─────────┼────────────┼────────────┼──────────────┼──────────────────┤
    │ Философия        │ 108      │ экзамен │ 48         │ экзамен    │ 36           │ Переатт.(частич.)│
    └──────────────────┴──────────┴─────────┴────────────┴────────────┴──────────────┴──────────────────┘
    """
    total_cols = 7
    total_rows = 2 + len(control_rows)  # 2 строки заголовка + данные
    ...
    for i, ct in enumerate(control_rows):
        ...
        # По учебному плану — форма контроля
        fc_norma_name = (
...
def group_students_by_uni_spec(
    ...
    Возвращает OrderedDict: ключ → отсортированный список студентов.
    """
    ...
    # Сортируем студентов внутри групп по ФИО
    for key in groups:
...
def get_unique_control_data(
    ...
    """
    Получить уникальные строки ControlTable для направления.
...
def generate_order_for_direction(
    ...
    """Генерирует Word-приказ для одного направления"""
    ...
    print(f"      Студентов: {len(students)}, групп: {len(groups)}, "
          f"предметов в таблице: {len(control_data)}")

    return filepath


# ══════════════════════════════════════════════
#  ГЛАВНАЯ ФУНКЦИЯ — ГЕНЕРАЦИЯ ВСЕХ ПРИКАЗОВ
# ══════════════════════════════════════════════

def generate_all_orders(session: Session, output_dir: str = 'output'):
    """
    Генерирует Word-приказы для всех IncomingDirection
    где is_used == False.

    После успешной генерации ставит is_used = True.
    """
    os.makedirs(output_dir, exist_ok=True)

    directions = (
        session.query(IncomingDirection)
        .filter(IncomingDirection.is_used == False)
        .all()
    )

    if not directions:
        print("📭 Нет новых направлений для генерации приказов")
        return []

    print(f"{'=' * 60}")
    print(f"Направлений для генерации: {len(directions)}")
    print(f"{'=' * 60}\n")
    ...
    for direction in directions:
        print(f"📝 Направление: {direction.name[:70]}...")

        try:
            filepath = generate_order_for_direction(
                session, direction, output_dir
            )
            ...
            print(f"   ❌ ОШИБКА: {e}\n")
            errors.append((direction.name, str(e)))
            session.rollback()

    # ── Итоги ──
    print(f"\n{'=' * 60}")
    print(f"Сгенерировано: {len(generated)}/{len(directions)}")
...
if __name__ == '__main__':
    ...
    with SessionLocal() as session:
        generate_all_orders(session, output_dir='output_orders')